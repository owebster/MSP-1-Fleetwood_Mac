...
For my first project I wanted to focus on building my site to be as user friendly as possible. Making sure different elements on the site act as they should and look as they should.
For example, when a user typically sees an icon on screen (Music streaming icons or social media icons), they would expect for it to do something or take them somewhere that is relevant to the content they are looking at, so even the small twitter icons on the latest tweets section would take you to the relevant tweet, not just twitter.com in general or how the music streaming icons take you to the relevant site on desktop or how on mobile, it will take the user to the correct application (Apple Music to Music App, Spotify to the Spotify App and YouTube to the YouTube App).
Another part of the site I focused on was the journey between different sections. This lead to always having the navigation bar at the top of the site (by using the sticky-top functionality) to allow users to jump to different sections no matter what part of the site they're on. This scales over all screen sizes, no matter if you're on desktop or mobile. With this choice, I also made the decision to hide the main logo from the page as you scroll down to let users focus on the content of the site with the most space as possible (meaning mobile/small screen users will not need to worry about a big logo taking up most of the space).

### Journey of the Site:
Following on with the experience of the site, my main focus of building the site was to ensure I had a clear journey for users when visiting the site. This meant that each section flowed with each other and made sense to be one after the other. When someone visits the site ->

   * The navbar is going to always be on top to allow users to easily jump between different sections.

   * They are greeted with the hero carousel, where they can see the latest albums as well as any news about the band.
   
   * Then as they move down, if they any of the hero carousel catches their attention, they are then greeted with different options to link to the band's streaming services (which mentioned above take users either to the appropriate website or mobile application via the `_blank` target).

   * Users are then greeted with the band's latest tour details, with where and when they're playing. This can help entice users to seek out and hopefully purchase tickets (ticket sales feature to be added in the future) to the band's shows, creating excitement for the user and increases the band's ticket sales.
   ...
   * Following on from the Sample section, users can see any latest news via the Band's twitter account (directly on the site, but also with associated links to the corresponding tweets - again with the `_blank` target to ensure users can come back to the site easily even if they go to the social media page)

   * Second to last section on the page encourages users to "Sign Up" for the band's newsletter for more information about upcoming tours and albums. This allows users to keep up to date but also will bring users back to the site as Newsletters / updates will direct users back to the webpage (actual newsletter feature to be added in the future).
...
### Colours:
Part of my focus when thinking about UX was the colour scheme as well. I wanted to create a sense of familiarity for users who have seen the latest albums and to associate these colours into new user's minds to remind them of the site when they eventually see the hero image of the site (which is the latest album). 
These colours are also good contrasting colours, so accessibility customers can still navigate through out the site without interruptions. Because of the contrast of the colours, it also helps with creating a sense of division between each section. 
The primary colours I used are:
   * Deep Red     : #8E191C   - This is being used for the - Hero Logo, Nav Bar, Text (on light gold background) and Section Blocks on alternating sections
...
This is shown in different areas of the site:

   * The Header / Nav bar are rounded off with corner radius and shadow to create a layer sort of effect, where the areas that are Deep Red (#8E191C) are floating on-top of the background (#fffcea). This is also seen with the alternating sections (Tour dates and latest tweets) where they have all four corners rounded off as well.

   * When the user scales up from mobile view, they don't just see a larger image in the hero carousel, they are given a similar image but with more content where possible (different style album artwork or different image entirely). This is done so the images are not just "larger" more stretched out versions of the ones seen on a mobile site.

   * Nav bar also expands when going from a small screen to medium screen to large screen, expanding the hidden sections from the dropdown menu to be on the navbar itself to utilise the larger space on larger screen devices (desktops, laptops or suitable tablets).

   * Smooth scrolling is enabled for the entire page, mobile and desktop. However Safari does not support this features without extra javascript (a feature that I wish to add in the future).

   * Columns also change depending on screen size, on mobile most sections utilise `col-12` to make sure not too much is on screen at one go. Then on tablet and desktop they respectably change to `col-6`, `col-4` or `col-3` depending on the content. (seen on Tour section, Music Sample section and Twitter section).
...
These are some of the considerations I have taken as a site owner:
   ...
   * To have a website that has colours in which are memorable and easily identifiable as the band's latest colours. As well as being easy on the eye and not too jarring
   * To stand out compared to other bands, by being modern and:
      * To offer a one stop shop for everything related to the band, meaning that the customer does not need to go to different websites to get different information (tour details, social media and music all under one site)
...
And for the user:
   ...
   * Streaming services links allow users to easily add and listen to music in popular streaming services, taking them directly to the app on mobile and taking them to the corresponding site on desktop
   * Tour section is a clean and easy to understand when and where the band will be performing next
   * Music sample section showcases the band's latest and greatest hits within the site, allowing users to explore new music without having to leave the website
   * Tweet section shows that the band is on social media and shows latest tweets to help keep users up to date
   * Newsletter section provides a clear and straightforward signup process (minimal amount of details needed, meaning that users wont feel like they have to give up a lot of privacy just to sign up to a newsletter)
...
### Start
   ...
   * Research and planning: 
      ...
      * From creating the skeleton, I looked at [Bootstrap](https://getbootstrap.com/) for different component ideas (nav bar styles, buttons, tables)
   ...
   * UI and UX planning
      * Beginning with the basic HTML layout, I laid out: 
         ...
         * The 4 separate articles and sections within the body (Tours, Samples, Tweets & Newsletter. Tweets was formally known as socials)
      ...
      * I thought about the "story" and flow of the site (mentioned above) and planned out where each section would be and how it might look.
         * I used CSS to pair and customise different colours and found colours via [ColorTools](https://www.colortools.net/color_complementary.html), which allowed me to analyse similar colours and contrasting within the same colour pallet.
      * I took what I had learnt around Bootstrap and imported the framework to help give some context to the website.

### Middle

   * Content
      * As I had laid out and chose the basic colour scheme of the site, I started brining in the content that I wanted to include:
         * Album art work for hero carousel (also included larger styled artwork for large screen devices)
         * Soundcloud integration (via Soundcloud's own embedding options via their site)
   ...
   * Styling
      * Finalised on smaller colour choices for text in different sections (making sure there is a good enough contrast difference between background colours and chosen text colours)
...
### Ongoing
   ...
   * With this ongoing section, this is where I have idea's to build into the site that I might not have had the time to do right now, or have not learnt the appropriate technologies to implement.
      * Hover styling for Tour section (currently implemented hover states for each line, however have not yet implemented a “sold out" hover style on tour dates that have been sold out)
      ...
      * Performance improvements (ongoing: compressing image sizes, refinements in code)

## User Stories
### Current Fan
I love the band and have been listening to them since I was a child with my parent. I care about and enjoy the fact that the site has everything I need to keep updated to the band and what is happening next, without me having to go to different websites for individual bits of information
...
### Current:
   * Live carousel for latest Albums
      ...
      * When in medium/large (desktop) view, this shows larger images to fit on screen and fill the screen more, offering a more immersive look
   * Tour table, showcasing upcoming tours
      * When in small/mobile, this is displayed with one country section above the other to offer a less cramped experience
   ...
   * Latest tweet section with cards from Bootstrap
   ...
   * Newsletter signup section
      * Adaptive to different screen sizes, offering same layout across different screen sizes
   ...
   * The main header enlarges as device screen enlarges from small -> medium -> large to offer the best viewing experience by not taking up too much space on each respected screen size
...
### Future:
   ...
   * Adding Javascript to offer:
      ...
      * Shop and cart (for merchandise store)
      * Different styling effects for site (replacing some bootstrap e.g. on the navbar)
   * Store to purchase concert tickets and merchandise
   * Live twitter integration (removes the need for manual editing of tweets)
...
<img src="assets/images/w3_validation.png" alt="screenshot of CSS w3 Validation completion checks, passed" width="576" height="360" />
...
## Deployment:
During this project, I made sure that I spent some of my time focusing on how to properly manage and deploy my project.
I used my own IDE, Github's desktop app and the IDE's (VS Code) terminal to create branches, test and deploy my code.
I primarily did my work in my testing branch as I knew that I would commit to my main branch closer to the time I made my site public.
Within the IDE:
